...
//class CosetCover
//{
//	const DistanceTable &phase_1, &phase_2;
...
int main()
{
	RandomTwistGenerator gen{ 928322 };
	for (int i = 0; i < 1'000'000; i++)
	{
		...
		c = c.twisted(twists);
		e = e.twisted(twists);
		auto c_prm = c.prm_index();
		auto e_prm = e.prm_index();
		auto c_even = is_even_permutation(c_prm);
		auto e_even = is_even_permutation(e_prm);
		std::cout << c_prm << " " << e_prm << " " << std::boolalpha << c_even << " " << e_even << std::endl;
	}
}