...
void BruteForceSolver::solve(const Cube3x3& cube, int depth, Cube3x3::Twist last)
{
	if (depth == 0)
	{
		if (cube.is_solved())
...
std::vector<Cube3x3::Twist> BruteForceSolver::solve(const Cube3x3& cube, int max_depth)
{
	twists.clear();
	try
	{
