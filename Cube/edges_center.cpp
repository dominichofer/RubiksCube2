...
{
	auto A = _mm_and_si128(a.state, _mm_set1_epi8(0x08));
	auto B = _mm_and_si128(b.state, _mm_set1_epi8(0x08));
	auto x = _mm_xor_si128(A, B);
	return _mm_test_all_zeros(x, x);
}
...
{
	auto A = _mm_and_si128(a.state, _mm_set1_epi8(0x04));
	auto B = _mm_and_si128(b.state, _mm_set1_epi8(0x04));
	auto x = _mm_xor_si128(A, B);
	return _mm_test_all_zeros(x, x);
}
