...
Twist twist_from_string(std::string_view);
...
// Concept that matches string-like types: std::string, std::string_view, const char[]
template<typename T>
concept string_like =
std::same_as<std::remove_cvref_t<T>, std::string> ||
std::same_as<std::remove_cvref_t<T>, std::string_view> ||
(std::is_array_v<std::remove_cvref_t<T>> &&
	std::is_same_v<std::remove_cvref_t<std::remove_all_extents_t<T>>, char>);
...
public:
	...
	template<std::ranges::range R>
		requires (!string_like<R>)
	Derived twisted(const R& twists) const
	{
		return twisted(twists.begin(), twists.end());
	}

	Derived twisted(std::string_view s) const
	{
		auto cube = static_cast<const Derived&>(*this);
	...
	template <typename Iterator, typename Sentinel>
	Derived twisted_inversed(Sentinel rbegin, Iterator rend) const
	{
		auto cube = static_cast<const Derived&>(*this);
		for (auto it = rbegin; it != rend; ++it)
			cube = cube.twisted(inversed(*it));
		return cube;
	}

	template<std::ranges::range R>
		requires (!string_like<R>)
	Derived twisted_inversed(const R& twists) const
	{
		return twisted_inversed(twists.rbegin(), twists.rend());
	}
